...
def ML_Regression(ENR):
    ...
    ############ ML model setup
    model = Sequential()
    # model.add(Dense(nnode, input_dim=nnode, activation='linear',use_bias=False))
    ...
    model.save("ForTesting.h5")

def LaterUse(ENR):
    import numpy as np
    import tensorflow as tf
    from keras.models import load_model
    model = load_model('ForTesting.h5')
    model.summary()
    [xtrain,ytrain,xtest,ytest] = ld.GetData(0.1,ENR,'QGSP_FTFP_BERT_EML')

    predict = model.predict(xtest)
    EvalOut = TFile("ForTesting.root","RECREATE")
